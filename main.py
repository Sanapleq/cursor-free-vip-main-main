...
class Translator:
    def __init__(self):
        ...
        self._load_config_and_language()
    ...
    def _load_config_and_language(self):
        """Load config and language settings"""
        try:
            config = self._get_config()

            # Create language cache directory if it doesn't exist
            if config and config.has_section('Language'):
                self.language_cache_dir = config.get('Language', 'language_cache_dir')
                os.makedirs(self.language_cache_dir, exist_ok=True)

                # Set fallback language from config if available
                if config.has_option('Language', 'fallback_language'):
                    self.fallback_language = config.get('Language', 'fallback_language')

                # Load saved language from config if available, otherwise detect system language
                if config.has_option('Language', 'current_language'):
                    saved_language = config.get('Language', 'current_language')
                    if saved_language and saved_language.strip():
                        self.current_language = saved_language
                    else:
                        self.current_language = self.detect_system_language()
                        # Save detected language to config
                        self._save_language_to_config(self.current_language)
                else:
                    self.current_language = self.detect_system_language()
                    self._save_language_to_config(self.current_language)
            else:
    ...
    def _save_language_to_config(self, language: str):
        """Save language setting to config file"""
        try:
            config = self._get_config()
            if config and config.has_section('Language'):
                config.set('Language', 'current_language', language)
                config_dir = os.path.join(get_user_documents_path(), ".cursor-free-vip")
                config_file = os.path.join(config_dir, "config.ini")
                with open(config_file, 'w', encoding='utf-8') as f:
                    config.write(f)
        except Exception as e:
            logger.warning(f"Error saving language to config: {e}")
