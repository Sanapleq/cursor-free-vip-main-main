...
class MachineIDResetter:
    ...
    def reset_machine_ids(self):
        ...
        try:
            ...
            # First check if file is accessible at all
            can_read = os.access(self.db_path, os.R_OK)
            
            if not can_read:
                print(f"{Fore.RED}{EMOJI['ERROR']} {self.translator.get('reset.no_permission')} (Read){Style.RESET_ALL}")
                print(f"{Fore.YELLOW}File path: {self.db_path}{Style.RESET_ALL}")
                print(f"{Fore.YELLOW}Please run as Administrator{Style.RESET_ALL}")
                return False
            
            # Try to actually open the file for reading to verify access
            try:
                with open(self.db_path, 'r', encoding='utf-8-sig') as f:
                    pass  # Just test if we can open it
            except PermissionError as pe:
                print(f"{Fore.RED}{EMOJI['ERROR']} {self.translator.get('reset.no_permission')} (PermissionError: {pe}){Style.RESET_ALL}")
                print(f"{Fore.YELLOW}File path: {self.db_path}{Style.RESET_ALL}")
                print(f"{Fore.YELLOW}Please close Cursor and run as Administrator{Style.RESET_ALL}")
                return False
            except Exception as e:
                # Other errors are OK - we can still try to proceed
                print(f"{Fore.YELLOW}{EMOJI['WARNING']} Warning opening file: {e}{Style.RESET_ALL}")

            # Test actual write access by trying to append
            can_write = False
            try:
                with open(self.db_path, 'a', encoding='utf-8') as test_f:
                    pass
                can_write = True
            except PermissionError:
                print(f"{Fore.RED}{EMOJI['ERROR']} {self.translator.get('reset.no_permission')} (Write){Style.RESET_ALL}")
                print(f"{Fore.YELLOW}File path: {self.db_path}{Style.RESET_ALL}")
                print(f"{Fore.YELLOW}Please close Cursor application and try again{Style.RESET_ALL}")
                print(f"{Fore.YELLOW}Or run: attrib -R \"%APPDATA%\\Cursor\\User\\globalStorage\\storage.json\"{Style.RESET_ALL}")
                return False
            ...
            for encoding in ['utf-8-sig', 'utf-8']:
                try:
                    with open(self.db_path, "r", encoding=encoding) as f:
                        config = json.load(f)
                    print(f"{Fore.GREEN}{EMOJI['SUCCESS']} Read with encoding: {encoding}{Style.RESET_ALL}")
                    break
                except (UnicodeDecodeError, json.JSONDecodeError) as e:
                    last_error = e
                    continue
                except PermissionError as pe:
                    print(f"{Fore.RED}{EMOJI['ERROR']} Permission denied when reading file{Style.RESET_ALL}")
                    print(f"{Fore.YELLOW}Please close Cursor application{Style.RESET_ALL}")
                    return False
            
            if config is None:
                print(f"{Fore.RED}{EMOJI['ERROR']} Failed to read config file with any encoding{Style.RESET_ALL}")
                if last_error:
                    print(f"{Fore.YELLOW}Last error: {last_error}{Style.RESET_ALL}")
                return False
